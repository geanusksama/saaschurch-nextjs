-- Gerado por scripts/dump-baseline.mjs em 2026-09-24T17:33:38.072Z
...
create or replace view "public"."pastoral_timeline" as
 SELECT id,
    ...
    status,
    metadata
   FROM ( SELECT pastoral_visits.id,
        ...
        UNION ALL
         SELECT discipleships.id,
           ...
           FROM discipleships
          WHERE discipleships.deleted_at IS NULL) t
  WHERE ( SELECT fn_usuario_sistema() AS fn_usuario_sistema);
