-- App Igreja v3: perfil_id por gatilho BEFORE INSERT em vez de DEFAULT appv3_meu_perfil_id().
-- Regra do baseline (painelchurch/src/lib/baseline.ts): nenhum default de coluna depende de
-- função própria, porque 04_functions roda depois de 05_tables.
-- Cópia integral de appv3/app/supabase/appv3_schema.sql (idempotente).
...
-- Conta excluída pelo próprio membro (appv3_excluir_conta): o perfil fica
-- anônimo para segurar o histórico de dinheiro e de secretaria da igreja.
alter table public.appv3_perfis add column if not exists excluido_em timestamptz;
...
create table if not exists public.appv3_status (
  perfil_id uuid primary key references public.appv3_perfis(id) on delete cascade,
...
create table if not exists public.appv3_seguidores (
  seguidor_id uuid not null references public.appv3_perfis(id) on delete cascade,
...
-- Configuração do app por campo (painel Mobile › Configurações). Cada campo
-- tem sua sede (headquarters); o app mostra a escolhida aqui.
create table if not exists public.appv3_campo_config (
  ...
  headquarters_id uuid references public.headquarters(id) on delete set null,
  atualizado_em   timestamptz not null default now()
);
...
-- Categorias de evento e de produto (painel Mobile). O app continua lendo o
-- texto em appv3_eventos.categoria / appv3_produtos.categoria; esta tabela é a
-- lista de onde o painel escolhe, para ninguém digitar variações do mesmo nome.
-- ministry_id: o ministério dono da categoria (ex.: produtos dos Jovens).
create table if not exists public.appv3_categorias (
  id          uuid primary key default gen_random_uuid(),
  campo_id    uuid not null references public.campos(id) on delete cascade,
  tipo        varchar(10) not null check (tipo in ('EVENTO','PRODUTO')),
  nome        varchar(60) not null,
...
create unique index if not exists appv3_categorias_nome_uq on public.appv3_categorias(campo_id, tipo, lower(nome));

-- Categorias que já estavam em uso viram cadastro (idempotente).
insert into public.appv3_categorias (campo_id, tipo, nome)
select distinct e.campo_id, 'EVENTO', trim(e.categoria) from public.appv3_eventos e
 where e.campo_id is not null and nullif(trim(e.categoria), '') is not null
on conflict do nothing;
insert into public.appv3_categorias (campo_id, tipo, nome)
select distinct p.campo_id, 'PRODUTO', trim(p.categoria) from public.appv3_produtos p
 where nullif(trim(p.categoria), '') is not null
on conflict do nothing;
insert into public.appv3_categorias (campo_id, tipo, nome)
select distinct d.campo_id, 'PRODUTO', trim(d.categoria) from public.appv3_loja_destaques d
 where nullif(trim(d.categoria), '') is not null
on conflict do nothing;

-- -----------------------------------------------------------------------------
-- perfil_id de quem está logado, preenchido por gatilho (não por DEFAULT).
--
-- Antes era `default public.appv3_meu_perfil_id()`. O migrate-self das
-- igrejas cria as tabelas (05) ANTES das funções (04), então num banco sem o
-- app v3 o default apontava para uma função que ainda não existia e o
-- 05_tables inteiro era revertido (deploy da advilasolange, 2026-09-24). O
-- gatilho é criado no 10, depois das funções, e roda antes da checagem de
-- RLS — o app continua inserindo sem mandar perfil_id.
-- -----------------------------------------------------------------------------
...
declare t text;
begin
  foreach t in array array['appv3_status', 'appv3_seguidores', 'appv3_familiares', 'appv3_notificacao_leituras',
                           'appv3_solicitacoes', 'appv3_ebd_matriculas', 'appv3_jogos_pontos',
                           'appv3_biblia_salvos', 'appv3_biblia_anotacoes'] loop
    -- no banco que já tinha o default (referência), tira
    execute format('alter table public.%I alter column %I drop default', t,
                   case when t = 'appv3_seguidores' then 'seguidor_id' else 'perfil_id' end);
    execute format('drop trigger if exists %I on public.%I', t || '_perfil', t);
    execute format('create trigger %I before insert on public.%I for each row execute function public.appv3_preencher_perfil()', t || '_perfil', t);
  end loop;
end $$;

-- -----------------------------------------------------------------------------
-- Índices das listas do painel Mobile e do app (2026-09-24). Cada um cobre
-- um filtro + ordenação que as telas fazem; sem eles, a lista vira varredura
-- da tabela inteira quando o campo crescer.
-- -----------------------------------------------------------------------------
-- listas do painel: campo + mais recentes primeiro
create index if not exists appv3_solicitacoes_campo_data_idx  on public.appv3_solicitacoes(campo_id, criado_em desc);
create index if not exists appv3_contribuicoes_campo_data_idx on public.appv3_contribuicoes(campo_id, criado_em desc);
create index if not exists appv3_pedidos_campo_data_idx       on public.appv3_pedidos(campo_id, criado_em desc);
create index if not exists appv3_perfis_campo_data_idx        on public.appv3_perfis(campo_id, criado_em desc);
-- ingressos e reembolsos chegam ao campo pelo pedido/perfil
create index if not exists appv3_ingressos_pedido_idx   on public.appv3_ingressos(pedido_id, status);
create index if not exists appv3_ingressos_evento_idx   on public.appv3_ingressos(evento_id);
create index if not exists appv3_reembolsos_perfil_idx  on public.appv3_reembolsos(perfil_id, criado_em desc);
create index if not exists appv3_reembolsos_pedido_idx  on public.appv3_reembolsos(pedido_id);
create index if not exists appv3_reembolsos_ingresso_idx on public.appv3_reembolsos(ingresso_id);
create index if not exists appv3_reembolsos_status_idx  on public.appv3_reembolsos(status) where status = 'SOLICITADO';
-- "já vendeu?" antes de excluir evento/opção/produto
create index if not exists appv3_pedido_itens_evento_idx  on public.appv3_pedido_itens(evento_id);
create index if not exists appv3_pedido_itens_opcao_idx   on public.appv3_pedido_itens(opcao_id);
create index if not exists appv3_pedido_itens_produto_idx on public.appv3_pedido_itens(produto_id);
-- conteúdo por campo que ainda não tinha índice
create index if not exists appv3_loja_destaques_campo_idx on public.appv3_loja_destaques(campo_id);
create index if not exists appv3_ebd_turmas_campo_idx     on public.appv3_ebd_turmas(campo_id);
create index if not exists appv3_jogos_conteudo_campo_idx on public.appv3_jogos_conteudo(campo_id, jogo);
create index if not exists appv3_jogos_conteudo_licao_idx on public.appv3_jogos_conteudo(licao_id);
create index if not exists appv3_pix_config_campo_idx     on public.appv3_pix_config(campo_id, finalidade);
create index if not exists appv3_pix_config_mundial_idx   on public.appv3_pix_config(mundial_id);
-- alertas: o app lê as leituras da pessoa a cada abertura do sino
create index if not exists appv3_notificacao_leituras_perfil_idx on public.appv3_notificacao_leituras(perfil_id);
-- excluir conta procura as matrículas da pessoa
create index if not exists appv3_ebd_matriculas_perfil_idx on public.appv3_ebd_matriculas(perfil_id);
...
begin
  ...
  foreach t in array array['appv3_familiares','appv3_cartoes','appv3_contribuicoes','appv3_pedidos','appv3_ingressos',
                           ...
                           'appv3_biblia_salvos','appv3_biblia_anotacoes',
...
-- -----------------------------------------------------------------------------
-- Excluir conta (exigência da App Store e do Google Play). Apaga o que é só da
-- pessoa; o que é registro da igreja (dízimos, ofertas, pedidos, ingressos,
-- reembolsos e solicitações à secretaria) fica, ligado a um perfil anônimo.
-- O cadastro de membro (members) é da secretaria e não é tocado.
-- A conta de login é apagada, salvo quando é também usuário do saaschurch
-- (D-15: mesma conta de Auth) — aí só o perfil do app sai. Se o app antigo
-- deixou linhas presas à conta (FK sem cascata), a conta é anonimizada e
-- bloqueada em vez de apagada.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Alertas automáticos: quando a secretaria ou a tesouraria muda o status de
-- algo da pessoa (pelo painel Mobile), ela recebe o alerta no sino do app.
-- Só na troca de status; conta excluída não recebe.
-- -----------------------------------------------------------------------------
...
$$;
revoke all on function public.appv3_alertar(uuid, text, text, text, text) from public, anon, authenticated;
...
declare t text;
begin
  foreach t in array array['appv3_solicitacoes', 'appv3_contribuicoes', 'appv3_pedidos', 'appv3_reembolsos'] loop
    execute format('drop trigger if exists %I on public.%I', t || '_alerta', t);
    execute format('create trigger %I after update of status on public.%I for each row execute function public.appv3_alerta_status()', t || '_alerta', t);
  end loop;
end $$;
...
begin
  foreach f in array array[
    ...
    'appv3_faceid_status(uuid)',
    'appv3_atualizar_contato(text, text, text)',
    'appv3_familia_cadastro()',
    'appv3_excluir_conta()'] loop
...
create policy appv3_publico_troca on storage.objects for update to authenticated
  using (bucket_id = 'appv3-publico' and (storage.foldername(name))[1] = 'perfis'
         and (storage.foldername(name))[2] = auth.uid()::text);
-- Excluir conta: a pessoa apaga a própria foto de perfil e capa.
drop policy if exists appv3_publico_apagar on storage.objects;
create policy appv3_publico_apagar on storage.objects for delete to authenticated
  using (bucket_id = 'appv3-publico' and (storage.foldername(name))[1] = 'perfis'
         and (storage.foldername(name))[2] = auth.uid()::text);
