...
import { ArrowUpDown, Building2, Calendar, CheckCircle2, Clock3, Download, Gift, Loader2, Pencil, Plus, Printer, Search, Trash2, UserRound, X, XCircle } from 'lucide-react';
...
type ConsecrationTitleOption = {
  ...
  consecrationTypeKey?: string | null;
  serviceId?: number | null;
  serviceSigla?: string | null;
  serviceDescription?: string | null;
};

type ConsecrationQueueItem = {
  id: string;
  protocol: string;
  member?: { id: string; fullName: string; phone?: string | null; mobile?: string | null; ecclesiasticalTitle?: string | null } | null;
  church?: { id: string; name: string; code?: string | null } | null;
  ...
  nextConsecration?: ConsecrationSchedule | null;
  notes?: string | null;
};
...
type DashboardPayload = {
  ...
  stats: {
    ...
    nextConsecrationDate?: string | null;
  };
};

type SortKey = 'member' | 'church' | 'service' | 'title' | 'status' | 'consecrationDate' | 'openedAt';
...
function statusClass(columnIndex: number) {
  if (columnIndex === 4) return 'bg-green-100 text-green-700';
  if (columnIndex === 5) return 'bg-rose-100 text-rose-700';
  if (columnIndex === 3) return 'bg-amber-100 text-amber-700';
  if (columnIndex === 2) return 'bg-blue-100 text-blue-700';
...
export function Consecration() {
  ...
  const [titleOptions, setTitleOptions] = useState<ConsecrationTitleOption[]>([]);
  ...
  const [editingRequest, setEditingRequest] = useState<ConsecrationQueueItem | null>(null);
  const [editingSchedule, setEditingSchedule] = useState<ConsecrationSchedule | null>(null);
  ...
  // Nome em processamento: fica na tela desde o envio até a linha aparecer na lista.
  ...
  async function loadTitleOptions() {
    ...
    try {
      ...
      setTitleOptions(rows);
      return rows as ConsecrationTitleOption[];
    } catch (loadError) {
      setTitleOptions([]);
      throw loadError;
    } finally {
  ...
  const visibleRows = useMemo(() => {
    ...
    const rows = (dashboard?.queue || []).filter((item) => {
      ...
      const textMatch = !query || normalizeText(`${item.member?.fullName || ''} ${item.protocol} ${item.church?.name || ''} ${item.service?.description || ''} ${item.currentTitle || ''} ${item.intendedTitle || ''} ${item.notes || ''}`).includes(query);
    ...
    return [...rows].sort((left, right) => {
      const leftValue = (() => {
        ...
        if (sortKey === 'title') return left.intendedTitle || '';
        if (sortKey === 'status') return left.statusLabel || '';
        if (sortKey === 'consecrationDate') return left.consecrationDate || left.nextConsecration?.scheduledDate || '';
      ...
      const rightValue = (() => {
        ...
        if (sortKey === 'title') return right.intendedTitle || '';
        if (sortKey === 'status') return right.statusLabel || '';
        if (sortKey === 'consecrationDate') return right.consecrationDate || right.nextConsecration?.scheduledDate || '';
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      thisMonth: visibleRows.filter((row) => {
        const value = row.consecrationDate || row.nextConsecration?.scheduledDate || row.openedAt || null;
        if (!value) return false;
        const date = new Date(value);
        return !Number.isNaN(date.getTime()) && date.getMonth() === now.getMonth() && date.getFullYear() === now.getFullYear();
      }).length,
      pending: visibleRows.filter((row) => row.columnIndex === 1).length,
      completed: visibleRows.filter((row) => row.columnIndex === 4).length,
    };
  }, [visibleRows]);
  ...
  function handleSort(key: SortKey) {
    ...
    setSortKey(key);
    setSortDirection(key === 'member' || key === 'church' || key === 'service' || key === 'title' || key === 'status' ? 'asc' : 'desc');
  }

  function openScheduleModal(schedule?: ConsecrationSchedule | null) {
  ...
  async function openRequestModal(schedule?: ConsecrationSchedule | null, request?: ConsecrationQueueItem | null) {
    let loadedTitles = titleOptions;
    if (!loadedTitles.length) {
      try {
    ...
    setSelectedMember(
      request?.member?.id
        ? { id: request.member.id, fullName: request.member.fullName, churchId, church: request.church || null, ecclesiasticalTitle: request.currentTitle || null }
        : null,
    );
    setRequestForm({
      ...EMPTY_REQUEST_FORM,
      ...
      consecrationDate: toDateInputValue(request?.consecrationDate || defaultSchedule?.scheduledDate || ''),
      notes: request?.notes || '',
    });
  ...
  function handleChurchSelected(church: ChurchOption) {
    ...
    if (churchPickerTarget === 'request') {
      ...
      setRequestForm((current) => ({
        ...
        churchId: church.id,
        memberId: '',
        consecrationDate: toDateInputValue(fallbackSchedule?.scheduledDate || ''),
      }));
    }
  ...
  function handleMemberSelected(member: MemberOption) {
    ...
    setRequestForm((current) => ({
      ...
      memberId: member.id,
      consecrationDate: current.consecrationDate || toDateInputValue(fallbackSchedule?.scheduledDate || ''),
    }));
  ...
  async function submitRequest() {
    ...
    try {
      const response = await authFetch(`${apiBase}/consecration/requests${editingRequest ? `/${editingRequest.id}` : ''}`, {
        method: editingRequest ? 'PATCH' : 'POST',
        body: JSON.stringify({
          memberId: requestForm.memberId,
          titleId: requestForm.titleId,
          consecrationDate: requestForm.consecrationDate,
      ...
      // Mantém o indicador na tela até a lista ser recarregada com o registro.
      ...
      try {
        ...
        if (!landed) {
          toast.info('O registro foi criado, mas está fora dos filtros atuais da tela.');
        }
      } finally {
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      const endpoint = deleteTarget.kind === 'schedule'
        ? `${apiBase}/consecration/schedules/${deleteTarget.id}`
  ...
  async function handleBulkDeleteConfirm() {
    ...
    try {
      ...
      if (failed.length) {
        ...
        toast.error(`${deleted} registro(s) excluído(s), ${failed.length} falhou(aram).`);
      } else {
        toast.success(`${deleted} registro(s) de consagração excluído(s).`);
      }
      selection.clear();
      void qc.invalidateQueries({ queryKey });
    } catch (bulkError) {
  ...
  return (
    <div className="space-y-4 p-6 text-slate-900 dark:text-slate-100">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(180px,1fr)_minmax(135px,0.72fr)_minmax(135px,0.72fr)_minmax(150px,0.8fr)_minmax(120px,0.62fr)_minmax(125px,0.62fr)_minmax(125px,0.62fr)] xl:items-end">
          ...
          <div>
            ...
            >
              ...
              <option value="3">Documentos Aprovados</option>
              <option value="4">Consagrado</option>
      ...
      {(loading || loadingFilters) ? (
        <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
          <span className="inline-flex items-center gap-2">
            <Clock3 className="h-4 w-4 animate-pulse text-purple-600" />
            Carregando consagracoes e filtros...
          </span>
        </div>
      ) : null}
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
        {insertingLabel ? (
          <div className="flex items-center gap-3 border-b border-purple-200 bg-purple-50 px-4 py-3 text-sm text-purple-800">
            <Loader2 className="h-4 w-4 shrink-0 animate-spin text-purple-600" />
            <span>
              Incluindo <strong className="font-semibold uppercase">{insertingLabel}</strong> na consagração e atualizando a lista...
            </span>
          </div>
        ) : null}
        {selection.count > 0 ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1660px]">
            ...
            <tbody className="divide-y divide-slate-200">
              {loading || loadingFilters ? <TableLoadingRows columns={11} /> : null}
                ...
                <tr className="bg-purple-50/60">
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold uppercase text-purple-900">{insertingLabel}</p>
                        <p className="text-xs text-purple-700">Gerando protocolo...</p>
                      </div>
                    </div>
                  </td>
                  {Array.from({ length: 9 }).map((_, columnIndex) => (
                    <td key={columnIndex} className="px-4 py-4">
                      <div className="h-4 animate-pulse rounded bg-purple-100" />
                    </td>
                  ))}
                </tr>
              ) : null}
              {!loading && !loadingFilters ? paginatedRows.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-4 align-top text-sm text-slate-600">{formatDate(item.consecrationDate || item.nextConsecration?.scheduledDate)}</td>
        ...
        <ModalShell title={editingRequest ? 'Editar Consagração' : 'Iniciar Consagração'} onClose={() => {
          ...
          <div className="mb-4">
            ...
            <button
              type="button"
              onClick={() => {
                if (canPickRequestChurch) {
                  setChurchSearch('');
                  setChurchPickerTarget('request');
                }
              }}
              disabled={!canPickRequestChurch}
      ...
      <ConfirmDialog
        ...
        message={`Confirma a exclusão de ${selection.count} registro(s) de consagração? Esta ação não pode ser desfeita.`}
      ...
      <PrintModal
        ...
        columnOptions={[
          ...
          { value: 'church', label: 'Igreja' },
          { value: 'service', label: 'Serviço' },
          { value: 'currentTitle', label: 'Título Atual' },
          { value: 'intendedTitle', label: 'Título Pretendido' },
          { value: 'status', label: 'Status' },
          { value: 'consecrationDate', label: 'Data' },
          { value: 'openedAt', label: 'Aberto em' },
        ]}
        defaultSort="member"
        onPrint={(orientation, sortBy, selectedColumns) => {
          const sorted = [...visibleRows].sort((a, b) => {
            const va = sortBy === 'member' ? (a.member?.fullName ?? '') : sortBy === 'church' ? (a.church?.name ?? '') : sortBy === 'service' ? (a.service?.description ?? '') : sortBy === 'intendedTitle' ? (a.intendedTitle ?? '') : sortBy === 'status' ? a.statusLabel : sortBy === 'consecrationDate' ? (a.consecrationDate ?? '') : (a.openedAt ?? '');
            const vb = sortBy === 'member' ? (b.member?.fullName ?? '') : sortBy === 'church' ? (b.church?.name ?? '') : sortBy === 'service' ? (b.service?.description ?? '') : sortBy === 'intendedTitle' ? (b.intendedTitle ?? '') : sortBy === 'status' ? b.statusLabel : sortBy === 'consecrationDate' ? (b.consecrationDate ?? '') : (b.openedAt ?? '');
          ...
          const allCols = [
            ...
            { label: 'Título Atual', key: 'currentTitle', width: '100px' },
            { label: 'Título Pretendido', key: 'intendedTitle', width: '110px' },
            { label: 'Status', key: 'status', width: '80px' },
            { label: 'Data Consagração', key: 'consecrationDate', width: '90px' },
          ...
          printReport({
            ...
            rows: sorted.map((r) => ({
              ...
              consecrationDate: r.consecrationDate ? new Date(r.consecrationDate).toLocaleDateString('pt-BR') : '—',
...
function NextConsecrationCard({
  ...
  schedule: ConsecrationSchedule | null;
  canManageSchedules: boolean;
  onCreate: () => void;
  onEdit: () => void;
  onDelete: () => void;
  ...
  return (
    <div className={`rounded-3xl border px-4 py-3 ${cardTone}`}>
      {!schedule ? (
        <div className="flex h-full min-h-[76px] items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Próxima Consagração</p>
        ...
        <div className="flex h-full min-h-[76px] items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-emerald-700 dark:text-emerald-300">Próxima Consagração</p>
