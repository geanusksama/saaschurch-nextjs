    ...
    <div className="p-6">
      ...
      <div className="mb-6 flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Serviços e ocorrências</h1>
            <p className="text-sm text-slate-500">Gerencie os serviços e configure a matriz de decisão</p>
          </div>
        </div>
        <button
      ...
      <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
          <span className="text-sm font-semibold text-slate-700">Lista de Serviços</span>
          {loadingStructure && (
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((svc) => {
                ...
                <tr key={svc.id} className="hover:bg-slate-50">
                  <td className="px-5 py-3 font-bold text-sm text-slate-800">{svc.sigla}</td>
                  <td className="px-5 py-3 text-sm text-slate-600">
                    <div className="space-y-2">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-medium text-slate-700">{svc.description}</span>
                      ...
                      <div className="space-y-1 text-xs text-slate-500">
                        <div>
                          <span className="font-semibold text-slate-600">Pipelines:</span> {previewList(structure?.pipelineNames ?? svc.pipelineNames)}
                        </div>
                        <div>
                          <span className="font-semibold text-slate-600">Etapas:</span> {previewList(structure?.stageNames ?? svc.stageNames)}
                        </div>
                        <div>
                          <span className="font-semibold text-slate-600">Colunas:</span> {previewList(structure?.columnNames)}
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3 text-sm text-slate-600">
                    <div className="grid grid-cols-2 gap-2">
                      <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                        <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Pipelines</div>
                        <div className="mt-1 text-sm font-bold text-slate-700">{structure?.pipelineCount ?? svc.pipelineCount ?? 0}</div>
                      </div>
                      <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                        <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Etapas</div>
                        <div className="mt-1 text-sm font-bold text-slate-700">{structure?.stageCount ?? svc.stageCount ?? 0}</div>
                      </div>
                      <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                        <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Colunas</div>
                        <div className="mt-1 text-sm font-bold text-slate-700">{structure?.columnCount ?? svc.columnCount ?? 0}</div>
                      </div>
                      <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                        <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Regras</div>
                        <div className="mt-1 text-sm font-bold text-slate-700">{structure?.ruleCount ?? svc.ruleCount ?? 0}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        onClick={() => onEdit(svc)}
                        className="rounded-md border border-slate-200 p-1.5 text-slate-500 hover:text-slate-900 hover:bg-slate-100"
                      >
    ...
    <div className="p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-bold text-slate-900">Matriz de decisão</h1>
      ...
      <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100">
          <span className="text-sm font-semibold text-slate-700">Regras de Decisão</span>
        </div>
        {loading ? (
                  ...
                  <td className="px-5 py-3">
                    {rule.stage?.pipeline?.name ? (
                      <span className="inline-flex items-center gap-1.5">
                        ...
                        <span className="text-xs text-slate-500">{rule.stage.name}</span>
                      </span>
                    ) : rule.stage?.name ? (
                      <span className="text-xs text-slate-500">{rule.stage.name}</span>
                    ) : "—"}
...
function YesNoBadge({ value }: { value: boolean }) {
  return value ? (
    ...
    <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-400 border border-slate-200">Não</span>
  );
}
