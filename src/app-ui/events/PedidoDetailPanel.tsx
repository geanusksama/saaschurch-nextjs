...
type OrderDetail = {
  ...
  event?: {
    ...
    tipoEvento?: string; descricao?: string | null; preco?: number;
    participants?: { id: string; nome: string; papel: string | null; fotoUrl: string | null; ordem: number }[];
    sectors?: { id: string; nome: string; preco: number; corHex: string | null; quantidade: number }[];
  } | null;
  items: OrderItem[];
  qrcodes: QRCode[];
...
type TransferStep = "cpf" | "confirm" | "success";
...
function TransferSection({
  ...
  const [step, setStep]           = useState<TransferStep>("cpf");
  const [cpf, setCpf]             = useState("");
  const [preview, setPreview]     = useState<MemberPreview | null>(null);
  const [notFound, setNotFound]   = useState(false);
  const [searching, setSearching] = useState(false);
  const [fromName, setFromName]   = useState(buyerName);
  ...
  const searchMember = async () => {
    ...
    try {
      ...
      if (d.member) { setPreview(d.member); setStep("confirm"); }
  ...
  const transferMut = useMutation({
    mutationFn: async () => {
      ...
      return r.json() as Promise<{ success: boolean; fromName: string; member: MemberPreview }>;
    },
    onSuccess: (d) => {
      setFromName(d.fromName ?? buyerName);
      setStep("success");
      onSuccess();
    },
  ...
  if (step === "cpf") return (
    <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-700 rounded-xl p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-indigo-700 dark:text-indigo-300">Presentear / Transferir Ingresso</h3>
      </div>
      <p className="text-xs text-indigo-600 dark:text-indigo-400">
        Digite o CPF do destinatário para buscar o membro cadastrado.
      </p>
  ...
  // ── Etapa 2: Confirmação ──────────────────────────────────────────────────
  if (step === "confirm" && preview) return (
    <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-700 rounded-xl p-4 space-y-4">
      <div className="flex items-center gap-2">
        <span className="text-lg">🎁</span>
        <h3 className="text-sm font-semibold text-indigo-700 dark:text-indigo-300">Confirmar Transferência</h3>
      </div>

      {/* De → Para */}
      <div className="space-y-2">
        <p className="text-[11px] font-medium uppercase tracking-wide text-slate-400">De (remetente)</p>
        <div className="flex items-center gap-2 bg-white dark:bg-slate-800 rounded-lg px-3 py-2 border border-slate-200 dark:border-slate-700">
          ...
          <span className="text-sm font-medium text-slate-700 dark:text-slate-300 truncate">{buyerName}</span>
        </div>
        ...
        <p className="text-[11px] font-medium uppercase tracking-wide text-slate-400">Para (destinatário)</p>
        <div className="flex items-center gap-2 bg-white dark:bg-slate-800 rounded-lg px-3 py-2 border border-indigo-200 dark:border-indigo-600">
          {preview.photoUrl ? (
            <img src={preview.photoUrl} alt={preview.fullName} className="w-8 h-8 rounded-full object-cover border border-slate-200 shrink-0" />
          ) : (
      ...
      <p className="text-xs text-orange-600 dark:text-orange-400">
        ⚠ Os QR Codes ativos serão cancelados. O novo titular deverá abrir o app para receber o ingresso.
      </p>

      <div className="flex gap-2">
        <button
          onClick={() => { setStep("cpf"); setPreview(null); }}
          className="flex-1 py-2 text-sm text-slate-600 dark:text-slate-300 border border-slate-300 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl transition-colors"
        >
          Voltar
        </button>
        <button
          onClick={() => transferMut.mutate()} disabled={transferMut.isPending}
          className="flex-1 py-2 text-sm font-medium bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl disabled:opacity-50 transition-colors"
        >
  ...
  // ── Etapa 3: Sucesso ──────────────────────────────────────────────────────
  return (
    <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-xl p-5 space-y-4 text-center">
      <div className="flex flex-col items-center gap-2">
        <span className="text-4xl">✅</span>
        <h3 className="text-base font-bold text-green-700 dark:text-green-300">Ingresso transferido!</h3>
      </div>
      <div className="text-sm text-slate-600 dark:text-slate-400 space-y-0.5">
        <p><span className="font-medium text-slate-700 dark:text-slate-300">De:</span> {fromName}</p>
        <p><span className="font-medium text-slate-700 dark:text-slate-300">Para:</span> {preview?.fullName}</p>
      </div>
      <button
        onClick={() => { setStep("cpf"); setCpf(""); setPreview(null); setNotFound(false); }}
        className="w-full py-2 text-sm font-medium bg-green-600 hover:bg-green-700 text-white rounded-xl transition-colors"
      >
        Concluir
      </button>
    </div>
...
export function PedidoDetailPanel({
  ...
  return (
    <>
                    ...
                    <section>
                      {!showTransfer ? (
                        ...
                        <div>
                          ...
                          <TransferSection orderId={orderId} buyerName={order.buyerName ?? "Titular atual"} onSuccess={invalidate} />
                        </div>
                      )}
                    </section>
                  )}
                  ...
                  <section className="pt-2">
                    {!showDeleteZone ? (
                      ...
                      <div className="bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-700 rounded-xl p-4 space-y-3">
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => setConfirmDialog({ title: "Excluir pedido", message: `Excluir PERMANENTEMENTE o pedido ${order.numeroPedido ?? orderId}? Esta ação não pode ser desfeita.`, onConfirm: () => deleteMut.mutate() })}
              ...
              {activeTab === "items" && (
                <div className="space-y-4">
                    ...
                    (() => {
                      ...
                      return (
                        <section>
                          ...
                          {order.status === "PAGO" && activeQRs.length > 0 && (
                            <button
                              onClick={() => setConfirmDialog({ title: "Regenerar QR Codes", message: "Os QR Codes atuais serão cancelados e novos serão gerados. Continuar?", onConfirm: () => qrMut.mutate() })}
                  ...
                  {["AGUARDANDO_PAGAMENTO", "EXPIRADO"].includes(order.status) && (
                    <section>
                      <button
                        onClick={() => setConfirmDialog({ title: "Cancelar pedido", message: `Cancelar o pedido ${order.numeroPedido}? Esta ação não pode ser desfeita.`, onConfirm: () => cancelMut.mutate("Cancelado pelo administrativo.") })}
                    ...
                    <>
                      {order.status === "PAGO" && (
                        <div className="flex justify-end">
                          <button onClick={() => setConfirmDialog({ title: "Regenerar QR Codes", message: "Os QR Codes atuais serão cancelados e novos serão gerados. Continuar?", onConfirm: () => qrMut.mutate() })}
                            disabled={qrMut.isPending}
      ...
      {/* ── Confirm Dialog ── */}
      {confirmDialog && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" onClick={() => setConfirmDialog(null)}>
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
          <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl p-6 w-full max-w-sm space-y-4" onClick={e => e.stopPropagation()}>
