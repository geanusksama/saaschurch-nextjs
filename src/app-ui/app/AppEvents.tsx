...
import { Ticket, Plus, Search, Pencil, Eye, Calendar, MoreHorizontal } from 'lucide-react';
...
export default function AppEvents() {
  ...
  const { data: events = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from('app_events')
        .select('id, nome, status, data_inicio, data_fim, gratuito, preco, capacidade_total, quantidade_disponivel, tipo_evento')
        .is('deleted_at', null)
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((ev: {
                ...
                gratuito: boolean; preco: number; tipo_evento: string;
              }) => {
