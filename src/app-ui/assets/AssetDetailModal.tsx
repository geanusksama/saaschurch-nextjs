...
import { Package, Edit, Loader2, QrCode, MapPin, Calendar, Tag, X, type LucideIcon } from 'lucide-react';
...
export function AssetDetailModal({ assetId, onClose, onEdit }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div
        className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto text-slate-900 dark:text-slate-100"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between p-5 border-b border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-11 h-11 bg-amber-100 dark:bg-amber-900/30 rounded-xl flex items-center justify-center flex-shrink-0">
              <Package className="w-5 h-5 text-amber-600" />
            </div>
            <div className="min-w-0">
              <h2 className="text-lg font-bold truncate">{asset?.name || 'Carregando...'}</h2>
              <p className="text-sm text-slate-500 font-mono">{asset?.code}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            {asset && onEdit && (
              <button onClick={() => onEdit(assetId)} className="flex items-center gap-2 px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium">
        ...
        <div className="p-5">
          {loading && <div className="flex items-center justify-center py-16 text-slate-500"><Loader2 className="w-6 h-6 animate-spin" /></div>}
          {!loading && error && <div className="text-red-600 py-8 text-center">{error}</div>}
          {!loading && asset && (
            <div className="grid md:grid-cols-3 gap-6">
              ...
              <div className="bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 text-center h-fit">
