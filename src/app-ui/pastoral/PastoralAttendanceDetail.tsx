...
export function PastoralAttendanceDetail({
  ...
  const userChurchId = loggedInUser.churchId || loggedInUser.church_id || null;
  const userCampoId = loggedInUser.campoId || loggedInUser.campo_id || null;
  const participantChurchId = attendance?.church_id || userChurchId || churchId;
  ...
  const { data: memberSearchResults = [], isLoading: searchLoading } = useQuery({
    queryKey: ['pastoral-participant-member-search', participantChurchId, userCampoId, participantSearch, participantRole],
    enabled: !!participantChurchId && participantSearch.length >= 2,
    queryFn: () => searchMembersForAttendance({
      search: participantSearch,
      profileType: loggedInUser.profileType || 'church',
      churchId: participantChurchId,
      campoId: userCampoId,
    }),
    staleTime: 10_000,
  });
  ...
  const addParticipantMutation = useMutation({
    ...
    onSuccess: async (newParticipant: PastoralParticipant) => {
                  ...
                  {responsibles.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {responsibles.map((p) => {
                        ...
                        const roleName = m?.ecclesiastical_title || PARTICIPANT_ROLE_LABELS[p.role];
                        const churchName = m?.churches?.name || 'Igreja';
                        const campoName = m?.campos?.name || 'Sem Campo';
                        const avatar = m?.photo_url;
                        const phone = m?.phone;
                        const statusText = m?.membership_status || 'ATIVO';
                  ...
                  {otherParticipants.length === 0 ? (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {otherParticipants.map((p) => {
                        const name = p.members?.full_name || '—';
                        return (
                          <span key={p.id}
