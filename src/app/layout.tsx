 ...
 * (Sistema → Home Pública). Cada igreja roda contra o seu próprio banco, então
 * a leitura aqui é uma consulta só, por chave única.
 */
/**
 * A home lê a configuração da igreja no banco a cada requisição.
 *
 * Sem isto o Next pode renderizar `/` estaticamente no build e congelar
 * título, descrição e favicon da igreja que existia na hora do deploy — a
 * troca na tela de configuração nunca apareceria. O app é uma SPA que busca
 * tudo por API, então não há ganho de estático a perder aqui.
 */
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    icons: {
      icon: icone,
      apple: cfg.faviconUrl || "/icons/apple-touch-icon.png",
    },
  };
}
