...
export async function POST(req: NextRequest) {
  return withAuth(req, async (user) => {
    ...
    const authUser = await prisma.user.findUnique({ where: { id: user.id }, select: { churchId: true, regionalId: true, campoId: true } });
    if (!authUser?.churchId) return NextResponse.json({ error: "Usuário não vinculado a uma igreja." }, { status: 403 });
