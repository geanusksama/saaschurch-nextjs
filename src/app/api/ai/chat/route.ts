...
14. NUNCA descreva uma distribuição que você não mediu. Se a contagem voltar ZERO, não escreva de memória quais status, meses ou igrejas "provavelmente" têm o resto: consulte de novo sem o filtro (ou use "agrupar_por") e responda com o que a ferramenta devolveu. Quando a resposta trouxer "statusExistentesNoPeriodo", os status que existem são EXATAMENTE aqueles — citar qualquer outro é inventar dado.${pediuTodasAsIgrejas(message)
  ? `
          ...
          function: {
            ...
            parameters: {
              ...
              properties: {
                ...
                status: { type: "string", description: "Filtra pela SITUAÇÃO do processo, como aparece nas telas do sistema: Pendente, Aprovado, Reprovado, Cancelado, Batizado, Consagrado, Concluído, Finalizado. Acento é opcional. A resposta traz sempre 'statusExistentesNoPeriodo' com a distribuição real — se der zero, olhe ali o que existe em vez de supor." },
