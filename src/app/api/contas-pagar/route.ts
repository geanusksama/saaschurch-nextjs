...
export async function GET(req: NextRequest) {
  return withAuth(req, async (user) => {
    ...
    if (status) where.statusGeral = { in: status };
    const aprovacao = listaOuNada("statusAprovacao");
    if (aprovacao) where.statusAprovacao = { in: aprovacao };
    const credorId = sp.get("credorId");
    if (credorId) where.credorId = credorId;
    const planoDeContaId = sp.get("planoDeContaId");
    if (planoDeContaId) where.planoDeContaId = planoDeContaId;
    const departamentoId = sp.get("departamentoId");
    if (departamentoId) {
      // "sem" filtra o histórico/lançamentos que não informaram departamento.
      where.departamentoId = departamentoId === "sem" ? null : departamentoId;
    }
    const bancoId = sp.get("bancoId");
    if (bancoId) where.bancoId = bancoId === "sem" ? null : bancoId;
    ...
    // Faixa de vencimento filtra pelo título que TEM parcela na faixa.
    ...
    if (vencDe || vencAte) {
      ...
      where.parcelas = { some: { dataVencimento: filtro } };
    }
