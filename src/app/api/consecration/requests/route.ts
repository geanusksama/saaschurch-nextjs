...
export async function POST(req: NextRequest) {
  return withAuth(req, async (user) => {
    ...
    const { memberId, intendedTitle, notes } = body;
    ...
    const service = await prisma.kanService.findFirst({ where: { isActive: true, serviceGroup: "CONSAGRACAO" } });
    if (!service) return NextResponse.json({ error: "consecration service not configured" }, { status: 404 });
    ...
    if (!stage) return NextResponse.json({ error: "consecration stage not configured" }, { status: 404 });
    ...
    const card = await prisma.kanCard.create({
      data: {
        ...
        status: "pendente", statusLabel: firstColumn.name, observations: notes || null,
        createdBy: user.id || null, metadata: { flowType: "consagracao" },
      },
    });
