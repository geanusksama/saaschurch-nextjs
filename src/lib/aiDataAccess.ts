...
const pelaCelula = (ids: string[]) => ({ cellGroup: { churchId: { in: ids } } })
const semRecorte = () => ({})
...
export const AI_TABELAS: Record<string, TabelaSpec> = {
  ...
  // ── Financeiro: contas a pagar e cadastros de apoio ─────────────────────
  // Tudo daqui para baixo é 'financeiro': só o assistente dessa especialidade
  // enxerga. São dados de dinheiro com nome de fornecedor, valor e vencimento —
  // o agente pastoral não tem o que fazer com eles.
  contas_pagar: {
    grupo: 'financeiro',
    modelo: 'ContaPagar',
    delegate: () => prisma.contaPagar,
    scope: porIgreja,
    descricao: 'Contas a pagar: a obrigação inteira, ainda não parcelada. status geral, valorTotal e o credor. As datas de vencimento estão nas PARCELAS, não aqui.',
    relacoes: { church: churchSelect, planoDeConta: true, credor: true, departamento: true, banco: true, parcelas: true },
  },
  parcelas_contas_pagar: {
    grupo: 'financeiro',
    modelo: 'ParcelaContaPagar',
    delegate: () => prisma.parcelaContaPagar,
    scope: porIgreja,
    descricao: 'Parcelas das contas a pagar — é AQUI que estão vencimento e saldo. Para "o que vence em X" ou "quanto está em atraso", consulte esta tabela, não contas_pagar.',
    relacoes: { church: churchSelect, contaPagar: true, pagamentos: true },
  },
  pagamentos: {
    grupo: 'financeiro',
    modelo: 'PagamentoParcela',
    delegate: () => prisma.pagamentoParcela,
    scope: porIgreja,
    descricao: 'Pagamentos feitos nas parcelas, inclusive parciais. Pagamento estornado tem estornadoEm preenchido e NÃO deve ser somado.',
    relacoes: { church: churchSelect, parcela: true, banco: true },
  },
  credores: {
    grupo: 'financeiro',
    modelo: 'Credor',
    delegate: () => prisma.credor,
    scope: porIgreja,
    descricao: 'Fornecedores e credores. Pode apontar para um membro (member) ou para outra igreja (igrejaFavorecida), quando o repasse é interno.',
    relacoes: { church: churchSelect, member: memberSelect, igrejaFavorecida: churchSelect, banco: true, contas: true },
  },
  plano_de_contas: {
    grupo: 'financeiro',
    modelo: 'PlanoDeConta',
    delegate: () => prisma.planoDeConta,
    scope: porIgreja,
    descricao: 'Catálogo de planos de conta (01.200 - DÍZIMOS, 02.148 - LUZ...). Serve para descobrir o nome exato antes de filtrar o livro caixa por plano.',
    relacoes: { contasPagar: true },
  },
  contas_bancarias: {
    grupo: 'financeiro',
    ...
    scope: porIgreja,
    descricao: 'Contas bancárias e caixas cadastrados, com saldo.',
    relacoes: { church: churchSelect, transactions: true },
  },
  categorias_financeiras: {
    ...
    scope: porIgreja,
    descricao: 'Categorias financeiras em árvore (parent/children).',
    relacoes: { church: churchSelect, parent: true, children: true },
  },
  transacoes: {
    grupo: 'financeiro',
    modelo: 'Transaction',
    ...
    scope: porIgreja,
    descricao: 'Movimentação das contas bancárias. NÃO confunda com livro_caixa: o livro caixa é o lançamento eclesiástico (dízimo, oferta, despesa) e é ele que responde perguntas de arrecadação.',
    relacoes: { church: churchSelect, account: true, category: true, donorMember: memberSelect, offerings: true },
  },
  ofertas: {
    grupo: 'financeiro',
    modelo: 'Offering',
    delegate: () => prisma.offering,
    scope: porIgreja,
    descricao: 'Ofertas registradas pelo módulo de transações. Para totais de oferta o caminho normal continua sendo o livro caixa.',
    relacoes: { church: churchSelect, member: memberSelect, transaction: true },
  },
  alugueis: {
    grupo: 'financeiro',
    modelo: 'ChurchRentRecord',
    delegate: () => prisma.churchRentRecord,
    scope: porIgreja,
    descricao: 'Contratos de aluguel do imóvel da igreja: valor, reajuste e vigência.',
    relacoes: { church: churchSelect },
  },
  pagamentos_stripe: {
    grupo: 'financeiro',
    modelo: 'StripePayment',
    delegate: () => prisma.stripePayment,
    scope: porIgreja,
    descricao: 'Pagamentos on-line pela Stripe (eventos, doações), com status e valor.',
    relacoes: { church: churchSelect, refunds: true },
  },
  assinaturas_stripe: {
    grupo: 'financeiro',
    modelo: 'StripeSubscription',
    delegate: () => prisma.stripeSubscription,
    scope: porIgreja,
    descricao: 'Assinaturas recorrentes na Stripe.',
    relacoes: { church: churchSelect },
  },
  ebd_financeiro: {
    grupo: 'financeiro',
    modelo: 'EbdFinanceiro',
    delegate: () => prisma.ebdFinanceiro,
    scope: porIgreja,
    descricao: 'Conta da EBD por igreja e trimestre: o que foi entregue em revistas e o que foi acertado.',
    relacoes: { church: churchSelect, trimestre: true, entrega: true, movimentos: true },
  },

  // ── Secretaria: vida da igreja ──────────────────────────────────────────
  ministerios: {
    grupo: 'pessoas',
    modelo: 'Ministry',
    delegate: () => prisma.ministry,
    scope: porIgreja,
    descricao: 'Ministérios e departamentos ministeriais (louvor, infantil...), em árvore, com o líder.',
    relacoes: { church: churchSelect, leader: memberSelect, parent: true, children: true, members: true },
  },
  ministerio_membros: {
    grupo: 'pessoas',
    modelo: 'MinistryMember',
    delegate: () => prisma.ministryMember,
    scope: peloMembro,
    descricao: 'Quem participa de cada ministério, com a função exercida.',
    relacoes: { ministry: true, member: memberSelect },
  },
  etiquetas: {
    grupo: 'pessoas',
    modelo: 'MemberTag',
    delegate: () => prisma.memberTag,
    scope: porIgreja,
    descricao: 'Etiquetas para marcar membros (visitante, novo convertido, afastado...).',
    relacoes: { church: churchSelect, cellGroup: true, assignments: true },
  },
  etiquetas_membros: {
    grupo: 'pessoas',
    modelo: 'MemberTagAssignment',
    delegate: () => prisma.memberTagAssignment,
    scope: peloMembro,
    descricao: 'Quais etiquetas cada membro recebeu.',
    relacoes: { member: memberSelect, tag: true },
  },
  gf_membros: {
    grupo: 'pessoas',
    modelo: 'CellGroupMember',
    delegate: () => prisma.cellGroupMember,
    scope: peloMembro,
    descricao: 'Participantes de cada grupo familiar/célula.',
    relacoes: { cellGroup: true, member: memberSelect },
  },
  gf_lideres: {
    grupo: 'pessoas',
    modelo: 'CellGroupLeader',
    delegate: () => prisma.cellGroupLeader,
    scope: peloMembro,
    descricao: 'Líderes de cada grupo familiar/célula.',
    relacoes: { cellGroup: true, member: memberSelect },
  },
  gf_reunioes: {
    grupo: 'pessoas',
    modelo: 'CellMeeting',
    delegate: () => prisma.cellMeeting,
    scope: pelaCelula,
    descricao: 'Reuniões dos grupos familiares, com data e presença.',
    relacoes: { cellGroup: true },
  },
  discipulado: {
    grupo: 'pessoas',
    modelo: 'DiscipleshipTrack',
    delegate: () => prisma.discipleshipTrack,
    scope: porIgreja,
    descricao: 'Trilhas de discipulado e suas lições.',
    relacoes: { church: churchSelect, lessons: true, enrollments: true },
  },
  discipulado_matriculas: {
    grupo: 'pessoas',
    modelo: 'DiscipleshipEnrollment',
    delegate: () => prisma.discipleshipEnrollment,
    scope: peloMembro,
    descricao: 'Quem está matriculado em cada trilha de discipulado, com progresso e mentor.',
    relacoes: { track: true, member: memberSelect, mentor: memberSelect },
  },
  eventos: {
    grupo: 'pessoas',
    modelo: 'Event',
    ...
    scope: porIgreja,
    descricao: 'Eventos e cultos programados (data, local, organizador). Inscrição e presença ficam nas tabelas eventos_inscricoes e eventos_presenca.',
    relacoes: { church: churchSelect, regional: true, registrations: true, attendance: true, sermons: true },
  },
  eventos_inscricoes: {
    grupo: 'pessoas',
    modelo: 'EventRegistration',
    delegate: () => prisma.eventRegistration,
    scope: peloMembro,
    descricao: 'Inscrições em eventos. Inscrito NÃO é o mesmo que presente — presença está em eventos_presenca.',
    relacoes: { event: true, member: memberSelect },
  },
  eventos_presenca: {
    grupo: 'pessoas',
    modelo: 'EventAttendance',
    delegate: () => prisma.eventAttendance,
    scope: peloMembro,
    descricao: 'Presença confirmada em evento — é esta que responde "quantos compareceram".',
    relacoes: { event: true, member: memberSelect },
  },
  leads: {
    grupo: 'pessoas',
    ...
    scope: porIgreja,
    descricao: 'Interessados/visitantes em acompanhamento, com origem e estágio.',
    relacoes: { church: churchSelect, member: memberSelect, activities: true },
  },
  pregacoes: {
    grupo: 'pessoas',
    modelo: 'Sermon',
    delegate: () => prisma.sermon,
    scope: porIgreja,
    descricao: 'Pregações registradas: tema, texto bíblico, data e pregador.',
    relacoes: { church: churchSelect, event: true, preacher: memberSelect },
  },

  // ── Cadastros que servem aos dois ───────────────────────────────────────
  contatos_igreja: {
    grupo: 'ambos',
    modelo: 'ChurchContact',
    delegate: () => prisma.churchContact,
    scope: porIgreja,
    descricao: 'Telefones, e-mails e responsáveis de contato da igreja.',
    relacoes: { church: churchSelect },
  },
  titulos_eclesiasticos: {
    grupo: 'ambos',
    modelo: 'EcclesiasticalTitle',
    delegate: () => prisma.ecclesiasticalTitle,
    scope: semRecorte,
    descricao: 'Catálogo de títulos eclesiásticos (Pastor, Presbítero, Diácono...). Use para descobrir o nome exato antes de filtrar membros por cargo.',
    relacoes: {},
  },
  patrimonio: {
    grupo: 'ambos',
    ...
    scope: porIgreja,
    descricao: 'Bens patrimoniais da igreja: código, descrição, valor e localização.',
    relacoes: { church: churchSelect, inventoryItems: true },
  },
  pipeline_colunas: {
    grupo: 'pessoas',
    modelo: 'KanColumn',
    delegate: () => prisma.kanColumn,
    scope: semRecorte,
    descricao: 'Colunas dos pipelines da secretaria — é o NOME da coluna que define a situação real do processo (Pendente, Aprovado, Cancelado). O campo status do card não acompanha a movimentação; use statusLabel do card ou esta tabela.',
    relacoes: { stage: true, cards: true },
  },
}
